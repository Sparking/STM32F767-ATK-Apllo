...
void FMC_sdram_write_buffer(uint8_t *pbuffer, uint32_t write_addr, uint32_t size)
{
...
void FMC_sdram_read_buffer(uint8_t *pbuffer, uint32_t read_addr, uint32_t size)
{
    for (uint32_t i = 0; i < size; i ++) {
...
void stm32f767_atk_apllo_sdram_init(void)
{
